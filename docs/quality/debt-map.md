...
- **[P1] Vercel Hobby runs only 2 cron jobs**, but `vercel.json` declares **5** — so Vercel silently runs only 2 and the other 3 (incl. `appointment-reminders` = the H-72/H-48 reminders) **never fire**. Verified 2026-06-26 via Vercel runtime logs: only `proposal-timeouts` + `interac-reminders` get traffic. Mitigation: the `.github/workflows/cron-sla.yml` GitHub Action pings **all 5** endpoints hourly — it **requires a repo Actions secret `CRON_SECRET`** equal to the Vercel env value (the workflow fails loudly if absent). Don't rely on the Vercel `crons` for full coverage on Hobby.
